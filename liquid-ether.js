/**
 * LiquidEther Vanilla JS Implementation
 * Ported from the React component.
 */

(function (global) {
  const THREE = global.THREE;
  if (!THREE) {
    console.error('LiquidEther requires THREE.js to be loaded.');
    return;
  }
  ...
  class ExternalForce extends ShaderPass {
    ...
    update(args) {
      const { mouse_force, cursor_size } = args;
  ...
  class Output {
    constructor(colors) {
      ...
      this.scene = new THREE.Scene(); 
      this.camera = new THREE.Camera();
      const paletteTex = makePaletteTexture(colors || ['#5227FF', '#FF9FFC', '#B19EEF']);
  ...
  global.LiquidEther = LiquidEtherManager;
